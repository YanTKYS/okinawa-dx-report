...
（113件）。`note` 列には「原資料の記載内容」と「ルーブリック条件への当てはめ理由」を併記している。
...
**補集合からの否定推定は行わない（2026-09-17 改訂）。**
初版では「取組ありの自治体が列挙されているから、列挙外は非選択＝確認済み0点」という
導出（旧・導出規則R3）を用いていたが、第5回Deep Researchは末尾で途切れており
列挙が網羅的である保証が無いため**廃止した**。該当28レコード
（`II-3`×7・`II-4`×9・`V-2`×6・`V-3`×6）は未確認へ差し戻している。
上記の総務省原Excelで自治体別の非選択を直接確認できた時点で、確認済み0点へ戻す。
...
| `II-3`・`II-4`・`V-2`・`V-3` の自治体別の非選択（28レコード） | 総務省個別資料（１）（２）／行政サービスの向上・高度化 |
